...
class WoodCraftSupabaseUser extends BaseAuthUser {
  WoodCraftSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> woodCraftSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = WoodCraftSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
